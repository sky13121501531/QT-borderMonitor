...
void VLCWrapperImpl::SetFullScreen(bool isfull)
{
	//set full screen ???程序中全屏
	libvlc_set_fullscreen(pMediaPlayer_,isfull?1:0);
}
...
void VLCWrapperImpl::SetWHScreen( const char *pWH )
{
    libvlc_video_set_aspect_ratio(pMediaPlayer_, pWH);
}

float VLCWrapperImpl::Getbit( void )
{
